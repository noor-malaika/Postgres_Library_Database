	...
	CREATE TABLE tbl_book (
		book_BookID INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY NOT NULL,
	...
	CREATE TABLE tbl_library_branch (
		library_branch_BranchID INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY NOT NULL,
	...
	CREATE TABLE tbl_borrower (
		borrower_CardNo INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY NOT NULL,
	...
	CREATE TABLE tbl_book_loans (
		book_loans_LoansID INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY NOT NULL,
	...
	CREATE TABLE tbl_book_copies (
		book_copies_CopiesID INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY NOT NULL,
	...
	CREATE TABLE tbl_book_authors (
		book_authors_AuthorID INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY NOT NULL,
...
CREATE FUNCTION bookCopiesAtAllSharpstown 
...
AS $$
begin
		return query(SELECT tbl_book_copies.book_copies_BranchID, tbl_library_branch.library_branch_BranchName, tbl_book_copies.book_copies_No_Of_Copies, tbl_book.book_Title
...
$$ language plpgsql;
SELECT * FROM bookCopiesAtAllSharpstown ('The Lost Tribe', 'Sharpstown');
...
SELECT * FROM bookCopiesAtAllBranches('Dune');
...
AS $$
BEGIN
RETURN(SELECT borrower_BorrowerName FROM tbl_borrower
	WHERE NOT EXISTS
		(SELECT * FROM tbl_book_loans
...
(DueDate VARCHAR(20) = NULL, LibraryBranchName varchar(50) = 'Sharpstown')
RETURNS TABLE(
	branchName VARCHAR(100),
	bTitle VARCHAR(100),
...
AS $$
BEGIN
	RETURN QUERY(SELECT tbl_library_branch.library_branch_BranchName,  tbl_book.book_Title,
	   tbl_borrower.borrower_BorrowerName, tbl_borrower.borrower_BorrowerAddress,
	   tbl_book_loans.book_loans_DateOut, tbl_book_loans.book_loans_DueDate
	   FROM tbl_book_loans
...
END;
$$ language plpgsql;
SELECT * FROM LoanersInfo('2/5/18', 'Central');
...
RETURNS TABLE(
	branchName VARCHAR(100),
	totalLoanBooks BIGINT
)
AS $$
BEGIN
	RETURN QUERY(SELECT  tbl_library_branch.library_branch_BranchName , COUNT (tbl_book_loans.book_loans_BranchID)
		FROM tbl_book_loans
			INNER JOIN tbl_library_branch ON tbl_book_loans.book_loans_BranchID = tbl_library_branch.library_branch_BranchID
			GROUP BY tbl_library_branch.library_branch_BranchName ORDER BY COUNT (tbl_book_loans.book_loans_BranchID) asc);
END;
$$ language plpgsql;
...
AS $$
BEGIN
	RETURN QUERY(SELECT tbl_borrower.borrower_BorrowerName, tbl_borrower.borrower_BorrowerAddress,
		   COUNT(tbl_borrower.borrower_BorrowerName)
		   FROM tbl_book_loans
		   			INNER JOIN tbl_borrower ON tbl_book_loans.book_loans_CardNo = tbl_borrower.borrower_CardNo
					GROUP BY tbl_borrower.borrower_BorrowerName, tbl_borrower.borrower_BorrowerAddress
		   HAVING COUNT(tbL_borrower.borrower_BorrowerName) >= BooksCheckedOut);
END;
$$ language plpgsql;
...
AS $$
BEGIN
	RETURN QUERY(SELECT tbl_library_branch.library_branch_BranchName, tbl_book.book_Title, tbl_book_copies.book_copies_No_Of_Copies
		   FROM tbl_book_authors
				INNER JOIN tbl_book ON tbl_book_authors.book_authors_BookID = tbl_book.book_BookID
				INNER JOIN tbl_book_copies ON tbl_book_authors.book_authors_BookID = tbl_book_copies.book_copies_BookID
				INNER JOIN tbl_library_branch ON tbl_book_copies.book_copies_BranchID = tbl_library_branch.library_branch_BranchID
			WHERE tbl_library_branch.library_branch_BranchName = BranchName AND tbl_book_authors.book_authors_AuthorName = AuthorName);
END;
$$ language plpgsql;
SELECT * FROM BookbyAuthorandBranch();


CREATE OR REPLACE FUNCTION Success()
RETURNS TRIGGER
AS $$
BEGIN
